...
class FeatureEngineer:
    def __init__(self, df, log_file, target_column):
        self.df = df
    ...
    def impute_missing_values(self, column, iterations):
        ...
        for i in range(iterations):
            ...
            errors_lr += [models.linear_regression(train_x, train_y, test_x, test_y.flatten(), None, True)]

            error_knn, k_neighbors, distance = models.similar_items(train_x, train_y, test_x, test_y.flatten(), True)
        ...
        if mean_avg_error <= lr_avg_error and mean_avg_error <= knn_avg_error:
            ...
            self.df.loc[self.df[column].isnull(), column] = helper.reverse_scale(mean_vals_scaled, scaler_target_col).flatten()
        elif lr_avg_error <= knn_avg_error:
            lr_vals_scaled, self.lr = models.linear_regression(train_x_full, train_y_full, test_x_full, None, None, False)
            self.df.loc[self.df[column].isnull(), column] = helper.reverse_scale(lr_vals_scaled, scaler_target_col).flatten()
            self.log_model(column, 'regressor', lr_avg_error)

        else:
            knn_vals_scaled, self.knn = models.similar_items(train_x_full, train_y_full, test_x_full,
                                                             None, False, int(np.average(knn_neighbors)),
                                                             mode(knn_distance))
            self.df.loc[self.df[column].isnull(), column] = helper.reverse_scale(knn_vals_scaled,
                                                                                 scaler_target_col).flatten()
            self.log_file.write("Column %s imputed with knn value (%d neighbors, %s distance metric). Mean squared error: %f \n"
                                % (column, self.knn.n_neighbors, self.knn.weights, knn_avg_error))

    def log_model(self, column, method, mean_avg_error):
        self.log_file.write("Column %s imputed with %s value. Mean squared error: %f \n" % (column, method, mean_avg_error))
